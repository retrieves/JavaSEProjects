...
public class LeaveStockDialog extends JDialog implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		case "confirm":
			...
			LeaveStock leaveStock = new  LeaveStock(new Date(),id );
			lsm.addLeaveStock(leaveStock);
			String leaveStockNo = leaveStock.getNo();
			int stockId = 0;
			int stockQty = 0;
			for(int i = 0 ; i< jt.getRowCount();i++) {
				if(((Boolean)jt.getValueAt(i, 3)).booleanValue() ) {
					...
					LeaveStockItem esi = new LeaveStockItem(leaveStockNo,stockId,stockQty);
					lsim.addLeaveStockItem(esi);
				}
			}
